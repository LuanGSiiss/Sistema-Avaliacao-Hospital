...
// Retorna uma string com o html das ações
function montarAcoes(idRegistro, rotaAlterar, rotaVisualizar, rotaExcluir) {
    acoes = `
        <a href="${rotaAlterar}${idRegistro}" class="botaoAcao alterar">Alterar</a>
        <a href="${rotaVisualizar}${idRegistro}" class="botaoAcao visualizar">Visualizar</a>
        <span onclick="mensagemExcluir(${idRegistro}, '${rotaExcluir}')" class="botaoAcao excluir">Excluir</span>
    `;
...
// Exclusão
function mensagemExcluir(idRegistro, rotaExclusao) {
    ...
    popMensagem.querySelector(".buttonSim").addEventListener("click", () => excluirRegistro(idRegistro, rotaExclusao));
    popMensagem.querySelector("#buttonNao").addEventListener("click", () => fecharMensagemExcluir());
}

function excluirRegistro(idRegistro, rotaExclusao) {
    ...
    func_assinc.open("DELETE", rotaExclusao + idRegistro, true);
